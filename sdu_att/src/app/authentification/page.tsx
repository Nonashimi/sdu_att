...
const page = () => {
    ...
    const checkUser = async (number: string, password: string) => {
        ...
        const user = users.find(user => user.number == number && user.pass == password);
        return user;
      }
  ...
  return (
    <div className="">
        <header className='flex justify-center border border-t-8 border-b-8 border-[#16296E] mt-8'>
            <Image src = {logo} width = {400} height = {148} alt = "logo"/>
        </header>
            ...
            <div className="auth_inner bg-[#f1f1f1] w-[100%] h-[100%] rounded-[4px] flex flex-col justify-center items-center">
                <div className="w-[80%]">
                    ...
                    <div className="w-[100%] mt-3">
                        <div className="text-[#005490] font-bold text-[14px]">Student Number</div>
                        <input value={user.number} onChange={(e) =>handleUserNumber(e.target.value)} type="text" className='w-[100%] border border-black outline-none px-[5px] py-[5px]' />
                    </div>
                    <div className="w-[100%]">
                        <div className="text-[#005490] font-bold text-[14px]">Password</div>
                        <input value={user.password} onChange={(e) => handleUserPassword(e.target.value)} type="password" className='w-[100%] border border-black outline-none px-[5px] py-[3px]' />
                    </div>
                    <button onClick={clickBtn} type = "button" className='border border-black rounded-[2px] text-[13px] py-[2px] px-[5px] mt-3 hover:bg-gray-300'>Log in</button>
                </div>
            </div>
        </div>
    </div>
